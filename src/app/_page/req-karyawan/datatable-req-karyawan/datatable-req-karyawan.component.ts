...
@Component({
  selector: 'app-datatable-req-karyawan',
  templateUrl: './datatable-req-karyawan.component.html',
  styleUrls: ['./datatable-req-karyawan.component.css']
})
export class DatatableReqKaryawanComponent implements OnInit, OnDestroy {
  ...
  approval(){
    const data = {
      ...
      flag_approval: this.dataApproval.flag_approval,
      description: this.dataApproval.description,
      user_proses: this.userData.email
    }
    this.ApiWithTokenService.add(data, 'requestKaryawan/approvalRequest', this.userData.token)
    .pipe(
      takeUntil(this.subs))
    .subscribe(res => {
      typeSuccess();
  ...
  update(ev){
    console.log(ev);
    this.router.navigate(['request-pegawai/edit/'+ev])
  }

  loker(ev){
    console.log(ev);
    this.router.navigate(['lowongan-kerja/add/'+ev])
  }
